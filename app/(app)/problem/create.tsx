...
import { Save, MapPin, Camera, File, ArrowLeft, X } from 'lucide-react-native';
...
export default function CreateProblemScreen() {
  ...
  const { createProblem, isLoading, error } = useProblemsStore();
  ...
  const handleCreateProblem = async () => {
    if (validateForm()) {
      try {
        const newProblem: Omit<Problem, 'id' | 'createdAt'> = {
          title: title.trim(),
        ...
        try {
          const problemRef = await createProblem(newProblem);
          console.log('Problema criado com sucesso, ID:', problemRef.id);
          
          // Criar objeto para exibição com ID
          const createdProblemWithId = {
            ...newProblem,
            id: problemRef.id,
            createdAt: Date.now()
          };
          
          setCreatedProblem(createdProblemWithId);
          setShowSuccess(true);
        } catch (uploadError) {
          console.error('Erro no upload ou criação:', uploadError);
          Alert.alert(
            'Erro no Upload', 
            'O problema foi reportado, mas houve um erro ao fazer upload das imagens. Tente novamente mais tarde.'
          );
        }
      } catch (error) {
        console.error('Error creating problem:', error);
        Alert.alert('Erro', 'Ocorreu um erro ao reportar o problema. Tente novamente.');
      }
    }
  };
  ...
  if (showSuccess) {
    return (
      <View style={styles.successContainer}>
        <Text style={styles.successTitle}>Problema Reportado com Sucesso!</Text>
        <View style={styles.successCard}>
          <Text style={styles.successCardTitle}>{createdProblem.title}</Text>
          <Text style={styles.successCardDescription}>{createdProblem.description}</Text>
          <Text style={styles.successCardLocation}>
            <MapPin size={16} color={Colors.gray[500]} /> {createdProblem.location}
          </Text>
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.push('/problems')} style={styles.backButtonIcon}>
      ...
      <Input
        label="Localização"
        placeholder="Informe onde o problema está localizado"
        value={location}
