...
export default function CreateServiceProviderScreen() {
  ...
  const { addServiceProvider, uploadProviderPhoto, isLoading } = useServiceProvidersStore();
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Create the service provider data object
      ...
      // Only add the photo property if we have a photo
      if (photo) {
        const photoUrl = await uploadProviderPhoto('temp', photo);
        if (photoUrl) {
          serviceProviderData.photo = photoUrl;
        }
      }
      
      const providerId = await addServiceProvider(serviceProviderData);
      
      if (!providerId) {
        throw new Error('Erro ao adicionar prestador de serviço');
      }
      
      Alert.alert(
        "Sucesso",
        "Prestador de serviço adicionado com sucesso!",
        [{ 
          text: "OK", 
      ...
      Alert.alert(
        "Erro",
        "Ocorreu um erro ao adicionar o prestador de serviço. Tente novamente.",
        [{ text: "OK" }]
      );
    } finally {
      setSubmitting(false);
    ...
    >
      <ScrollView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Adicionar Prestador</Text>
          <View style={{ width: 24 }} />
        </View>
          ...
          >
            {submitting ? (
              ...
              <View style={styles.submitButtonContent}>
                ...
                <Text style={styles.submitButtonText}>Adicionar Prestador</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>
