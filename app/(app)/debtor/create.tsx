...
export default function CreateDebtorScreen() {
  const { user } = useAuthStore();
  const { createDebtor, isLoading, error } = useDebtorsStore();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const debtorId = await createDebtor(debtorData);
      
      if (debtorId) {
        Alert.alert(
          'Sucesso',
          'Registro de inadimplência cadastrado com sucesso!',
          [{ text: 'OK', onPress: () => router.push('/debtors') }]
        );
      } else {
        Alert.alert('Erro', error || 'Não foi possível cadastrar o registro. Tente novamente.');
      }
    } catch (err) {
      console.error('Erro ao cadastrar inadimplente:', err);
      Alert.alert('Erro', 'Ocorreu um erro ao cadastrar o registro. Tente novamente.');
    } finally {
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: 'Cadastrar Inadimplente',
          headerLeft: () => (
            <TouchableOpacity 
      ...
      >
        <Text style={styles.subtitle}>
          Cadastre um novo morador inadimplente para acompanhamento.
        </Text>
